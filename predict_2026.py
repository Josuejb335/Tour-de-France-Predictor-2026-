...
Uses the XGBRanker trained on 2020-2025 data to predict each
stage's most likely winner, outputting top-5 per stage.
"""
...
# Manual elevation data from user
ELEVATIONS = {
    1: 200, 2: 2500, 3: 3850, 4: 2700, 5: 1600,
    6: 4100, 7: 850, 8: 1150, 9: 3300, 10: 3800,
    11: 1400, 12: 1800, 13: 2400, 14: 3800, 15: 3950,
    16: 500, 17: 2200, 18: 3900, 19: 3500, 20: 5450, 21: 1000,
}
...
# Build rider stats + encoding lookup
def best_val(series):
    s = series[series > 0]
    if len(s) > 0:
...
# Also load rider_data.json for additional lookups
...
if unmatched:
    print(f"Unmatched riders: {len(unmatched)} ({unmatched[:5]}...)")

# ──────────────────────────────────────────────────────────────────────────
# 3.  BUILD FEATURE ENCODERS & TRAIN MODEL
# ──────────────────────────────────────────────────────────────────────────
...
# Build name → encoding mapping from historical data
...
model.fit(X_scaled, y, group=group_counts, verbose=False)

# Latest temporal stats for each rider
...
# Encode using the pre-built maps, handling unseen riders
...
# Merge latest temporal stats by normalized name
...
for s in range(1, 22):
    ...
    sp = pred_df[mask].copy()
    sp_scores = sp['score'].values
    sp_probs = softmax(sp_scores)
    sp = sp.copy()
    sp['prob'] = sp_probs
    sp = sp.sort_values('prob', ascending=False)
...
out_df.to_csv('predictions_2026.csv', index=False)
print(f"\n{'='*80}")
